...
public class TestFrontend {
  ...
  public void runTests() {
    ...
    System.out.println("Test 'r' load ratings selection screen (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
    if(this.testFrontendRForRatings()){
      System.out.println("PASSED");
    } else {
      System.out.println("FAILED");
    }
    System.out.println("Test that program can't go immediately to genre mode from rating mode");
    if(this.testUnableToSwitchImmediately()){
      System.out.println("PASSED");
    } else {
      System.out.println("FAILED");
    }
  ...
  public boolean enterXToExit() {
    ...
    try {
      ...
      Object frontend = null; /* new Frontend(new Backend(new StringReader(
  ...
  public boolean testUnableToSwitchImmediately(){
    PrintStream standardOut = System.out;
    InputStream standardIn = System.in;
    try{
      // set the input stream to our input (with an g to test of the program lists genres)
      String input = "r" + System.lineSeparator() + "g" + System.lineSeparator() + "x" + System.lineSeparator() + "x";
      InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
      System.setIn(inputStreamSimulator);
      ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
      // set the output to the stream captor to read the output of the front end
      ...
      if (frontend != null && appOutput.contains("0")
                  ...
                  && appOutput.contains("9")
                  && appOutput.contains("10")) {
        // test passes if all ratings from the data are displayed on the screen instead of genres
        return true;
      } else {
        // test failed
        return false;
